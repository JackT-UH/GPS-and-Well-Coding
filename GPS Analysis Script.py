...
def process_file(file_path):
    try:
        ...
        for direction, column, sigma_column in [("UD (Vertical)", "UD_cm", "sigma_UD"), 
            ...
            trend_full = calculate_regression(df, x, column, data_in_cm=True, weights=weights, use_weighted=True)
            ...
            trend_smooth = calculate_regression(df_smooth, df_smooth["Year"].values, smooth_col, data_in_cm=True, weights=weights_smooth, use_weighted=True)
            ...
            if len(df_2018) > 2:
                ...
                trend_2018 = calculate_regression(df_2018, x_2018, column, data_in_cm=True, weights=weights_2018, use_weighted=True)
                ...
                if len(df_2018_smooth) > 2:
                    ...
                    trend_2018_smooth = calculate_regression(df_2018_smooth, df_2018_smooth["Year"].values, f'{column}_smooth_2018', data_in_cm=True, weights=weights_2018_smooth, use_weighted=True)
            ...
            y_deseas = df[deseas_col].values
            trend_deseas = calculate_regression(df, x, deseas_col, data_in_cm=True, weights=weights, use_weighted=True)
...
def calculate_regression(df, x, column, data_in_cm=False, weights=None, use_weighted=False):
    if len(x) < 3:
        return None
    ...
    if use_weighted and weights is not None:
        ...
        w_values = weights  # Keep in cm for weighting
        w = 1.0 / (w_values ** 2)
        w = w / np.sum(w) * len(w)  # Normalize
        
        # Weighted regression using polyfit (with data in mm for slope calculation)
        coeffs = np.polyfit(x, y_values, 1, w=w)
        slope = coeffs[0]  # mm/yr (already correct because y_values are in mm)
        intercept = coeffs[1]
...
def create_individual_plots(name, plots, save_to_dir, title_prefix=""):
    ...
    for i, (dates, values, trend, title, label) in enumerate(plots):
        ...
        if trend:
            ...
            ax.plot(dates, y_pred_cm, 'b-', linewidth=2, label=f"WLS Trend")
            ax.fill_between(dates, lower_cm, upper_cm, color="blue", alpha=0.2, label="95% CI")
