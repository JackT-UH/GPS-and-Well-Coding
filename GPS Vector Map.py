...
REF_VECTOR_MM_YR = 1.0              # vector scale arrow label/value
...
def main():
    ...
    # Vector scale arrow (bottom-left, labeled)
    ref_u_plot = (REF_VECTOR_MM_YR / 1000.0) * display_years
    qk = ax.quiverkey(
        q,
        X=0.12, Y=0.14, U=ref_u_plot,
        label=f"{REF_VECTOR_MM_YR:.0f} mm/yr",
        labelpos="E",
        coordinates="axes",
        color="black",
        labelcolor="black",
    ...
    qk.text.set_bbox(dict(facecolor="white", edgecolor="none", alpha=0.9, pad=0.3))
    qk.text.set_path_effects([pe.withStroke(linewidth=2.5, foreground="white")])
    ...
    cbar.set_label("Horizontal Magnitude (mm/yr)", fontsize=13, fontweight="bold", labelpad=12)
